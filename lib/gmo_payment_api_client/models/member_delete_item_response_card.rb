...
module GmoPaymentApiClient
  ...
  class MemberDeleteItemResponseCard
    # 会員ID
    attr_accessor :member_id

    # 削除されたカードの情報   `index`や`isDefault`は削除する前の情報です。   最新の状態は[取引照会API(/order/inquiry)](#tag/order/operation/orderInquiry)で確認してください。 
    ...
    def self.openapi_types
      {
        :'member_id' => :'String',
        :'deleted_card' => :'OnfileCardInformation'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GmoPaymentApiClient::MemberDeleteItemResponseCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GmoPaymentApiClient::MemberDeleteItemResponseCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'deleted_card')
        self.deleted_card = attributes[:'deleted_card']
      end
    end
