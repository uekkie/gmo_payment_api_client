...
module GmoPaymentApiClient
  ...
  class OnfileWallet
    ...
    # 登録されているPay払いのタイプ   登録されているPay払いのタイプによってパラメーターの指定要否が異なります。   本セクション冒頭の「共通パラメーター対応表」を参照してください。   - `PAYPAY`：PayPay - `DOCOMO`：d払い - `RAKUTEN`：楽天ペイ(オンライン決済)V2 - `AMAZON`：Amazon Pay V2 - `AU_APP`：au PAY(ネット支払い)アプリ方式 <span style=\"color: #d41f1c\">※本番環境では2024年3月1日から利用できます</span> - `MERPAY`：メルペイ 
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GmoPaymentApiClient::OnfileWallet` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GmoPaymentApiClient::OnfileWallet`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
