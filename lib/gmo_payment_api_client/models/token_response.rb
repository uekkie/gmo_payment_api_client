...
module GmoPaymentApiClient
  ...
  class TokenResponse
    ...
    # 有効期間(秒)   アクセストークンの有効期間を秒数で返却します。 
    attr_accessor :expires_in

    # スコープ   `openapi`を返却します。 
    attr_accessor :scope

    # アクセストークンのタイプ   `bearer`を返却します。 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GmoPaymentApiClient::TokenResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GmoPaymentApiClient::TokenResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      scope_validator = EnumAttributeValidator.new('String', ["openapi"])
    ...
    def scope=(scope)
      validator = EnumAttributeValidator.new('String', ["openapi"])
