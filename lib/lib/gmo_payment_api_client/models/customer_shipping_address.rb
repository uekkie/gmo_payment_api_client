  ...
  class CustomerShippingAddress
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GmoPaymentApiClient::CustomerShippingAddress` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GmoPaymentApiClient::CustomerShippingAddress`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@post_code.nil? && @post_code !~ pattern
    ...
    def valid?
      ...
      return false if !@post_code.nil? && @post_code !~ Regexp.new(/^[0-9-]+$/)
      true
    end
    ...
    def hash
      [name, line1, line2, line3, state, post_code, country, phone].hash
    end
